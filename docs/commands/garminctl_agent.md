## garminctl agent

AI-agent integration helpers

### Synopsis

Generate safety configuration for AI agents that drive garminctl.

### Options

```
  -h, --help   help for agent
```

### Options inherited from parent commands

```
      --dry-run          print the equivalent request instead of sending it
      --no-color         disable colored output
  -o, --output string    output format: table|json|yaml|csv (default "table")
      --profile string   profile (Garmin account) to use; env GARMINCTL_PROFILE
```

### SEE ALSO

* [garminctl](garminctl.md)	 - Drive Garmin Connect from the terminal
* [garminctl agent guard](garminctl_agent_guard.md)	 - Generate agent-safety config that blocks mutating garminctl operations

