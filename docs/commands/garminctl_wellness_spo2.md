## garminctl wellness spo2

Get blood oxygen (SpO2) for a date

### Synopsis

Get blood oxygen (SpO2) readings for a day including average, lowest, and sleep SpO2

```
garminctl wellness spo2 [date] [flags]
```

### Options

```
  -h, --help   help for spo2
```

### Options inherited from parent commands

```
      --dry-run          print the equivalent request instead of sending it
      --no-color         disable colored output
  -o, --output string    output format: table|json|yaml|csv (default "table")
      --profile string   profile (Garmin account) to use; env GARMINCTL_PROFILE
```

### SEE ALSO

* [garminctl wellness](garminctl_wellness.md)	 - wellness commands

