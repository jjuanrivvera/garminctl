## garminctl biometric lt-hr-range

Get lactate threshold HR range

### Synopsis

Get lactate threshold heart rate statistics for a date range

```
garminctl biometric lt-hr-range [flags]
```

### Options

```
      --end string     End date (YYYY-MM-DD)
  -h, --help           help for lt-hr-range
      --start string   Start date (YYYY-MM-DD)
```

### Options inherited from parent commands

```
      --dry-run          print the equivalent request instead of sending it
      --no-color         disable colored output
  -o, --output string    output format: table|json|yaml|csv (default "table")
      --profile string   profile (Garmin account) to use; env GARMINCTL_PROFILE
```

### SEE ALSO

* [garminctl biometric](garminctl_biometric.md)	 - biometric commands

