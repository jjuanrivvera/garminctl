## garminctl connect workouts unschedule

Unschedule a workout

### Synopsis

Remove a scheduled workout by its schedule ID

```
garminctl connect workouts unschedule <schedule_id> [flags]
```

### Options

```
  -h, --help   help for unschedule
```

### Options inherited from parent commands

```
      --dry-run          print the equivalent request instead of sending it
      --no-color         disable colored output
  -o, --output string    output format: table|json|yaml|csv (default "table")
      --profile string   profile (Garmin account) to use; env GARMINCTL_PROFILE
```

### SEE ALSO

* [garminctl connect workouts](garminctl_connect_workouts.md)	 - workouts commands

