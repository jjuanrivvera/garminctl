...
// newUpdateCmd builds `garminctl update` (self-update) + `update check`.
func newUpdateCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "update",
		Short: "Update garminctl to the latest release",
	...
	cmd.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if update.IsNewer(rel.TagName, cur) {
				fmt.Fprintln(out, "An update is available. Run `garminctl update` to install it.")
			} else {
