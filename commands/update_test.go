...
import (
	...
	"github.com/jjuanrivvera/garminctl/internal/update"
...
// TestUpdateCommand drives `update` (performs the update against a stub GitHub) and
// `update check` (reports availability) through a temporary binary — never the test binary.
func TestUpdateCommand(t *testing.T) {
	...
	asset := "garminctl_0.9.9_" + runtime.GOOS + "_" + runtime.GOARCH + "." + ext
	...
	tmpExe := filepath.Join(t.TempDir(), "garminctl")
...
// TestUpdateCommand_AlreadyLatest covers the "no newer release" branches: the current
// version is higher than what the server reports, so nothing is installed.
func TestUpdateCommand_AlreadyLatest(t *testing.T) {
	...
	version.Version = "9.9.9" // newer than the server's v0.9.9
	t.Cleanup(func() { newUpdater = origFactory; version.Version = origVer })
	...
	ck := newUpdateCmd()
	ck.SetOut(&bytes.Buffer{})
	ck.SetErr(&bytes.Buffer{})
	ck.SetArgs([]string{"check"})
	if err := ck.Execute(); err != nil {
		t.Fatalf("update check: %v", err)
	}
