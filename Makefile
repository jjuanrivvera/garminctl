# garminctl — Makefile. `make verify` is the deterministic gate (CI/dev);
# `make accept` (= verify + the LLM judge) is the build-acceptance gate.
BINARY      := garminctl
MODULE      := github.com/jjuanrivvera/garminctl
...
# It needs an agent and spends tokens, so it is NOT part of `verify` — run it only at
