...
func main() {
	// signal.NotifyContext makes Ctrl-C (SIGINT/SIGTERM) cancel in-flight work: token
	// refresh, retry backoff, and rate-limit waits all observe this context.
	...
	if err := root.ExecuteContext(ctx); err != nil {
		// Error text can carry an API-returned body; strip terminal escapes before printing
		// so a crafted value can't hijack the terminal.
