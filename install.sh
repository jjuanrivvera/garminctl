#!/bin/sh
# garminctl installer for macOS and Linux.
#
...
#   GARMINCTL_VERSION=v1.2.3   pin a version (default: the latest release)
...
REPO="jjuanrivvera/garminctl"
BINARY="garminctl"
VERSION="${GARMINCTL_VERSION:-}"
...
if [ -z "$VERSION" ]; then
  ...
  [ -n "$VERSION" ] || die "could not determine the latest release; set GARMINCTL_VERSION"
fi
